/*
  ...
  The circuit:
    ...
    Potentiometers connected to analog pin 3.
      Center of the pot goes to the analog pin.
      side pins go to +5V and ground.

     Potentiometers connected to analog pin 4.
      Center of the pot goes to the analog pin.
      side pins go to +5V and ground.
...
//libraries that the microcontroller references
#include <MozziGuts.h>
...
long fm_intensity; // carries control info from updateControl to updateAudio - no number assigned for now
...
//mapping the potentiometer values [0-1023] to a range - set above
...
//grab samples from out wav table
...
int updateAudio() {
  //library functions to generate the varying synth sounds
...
void loop() {
  //play the audio/do everything again
  audioHook();
}
